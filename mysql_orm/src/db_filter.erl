...
filter_string_format(?DB_CREATE_KEYS, QueryMap) -> %% 创建语句
    ...
    {ok, CFieldFormat, CKeyFormat} = db_parse:parse_fields(CreateFields, TotalFields),
...
filter_string_format(?DB_ADD_COLUMN, QueryMap) -> %% 添加表格字段
    ...
    case maps:get(?DB_ADD_COLUMN, QueryMap, []) of
        ...
        AddFields ->
            TotalFields = maps:get(?DB_TABLE_FIELDS, QueryMap, []),
            {ok, AFieldFormat, AKeyFormat} = db_parse:parse_fields(AddFields, TotalFields, "add column"),
            DropColFormat = io_lib:format("alter table ~s ~ts ~ts", [TableName, AFieldFormat, AKeyFormat])
    end,
    {ok, DropColFormat};
filter_string_format(?DB_MODIFY_COLUMN, QueryMap) -> %% 添加表格字段
    ...
    case maps:get(?DB_MODIFY_COLUMN, QueryMap, []) of
        ...
        AddFields ->
            TotalFields = maps:get(?DB_TABLE_FIELDS, QueryMap, []),
            {ok, AFieldFormat, AKeyFormat} = db_parse:parse_fields(AddFields, TotalFields, "modify column"),
            DropColFormat = io_lib:format("alter table ~s ~ts ~ts", [TableName, AFieldFormat, AKeyFormat])
    end,
    {ok, DropColFormat};
