...
add_modify_column_string(QueryMap) ->
    {ok, Fun} = db_filter:filter_string_format(?DB_ADD_MODIFY_COLUMN, QueryMap),
    {ok, Fun}.

%% 添加索引语句 orm
add_index_string(QueryMap) ->
    {ok, AddIndexFormat} = db_filter:filter_string_format(?DB_ADD_INDEX, QueryMap),
    {ok, AddIndexFormat}.

%% 删除索引语句 orm
drop_index_string(QueryMap) ->
    {ok, DropIndexFormat} = db_filter:filter_string_format(?DB_DROP_INDEX, QueryMap),
    {ok, DropIndexFormat}.
